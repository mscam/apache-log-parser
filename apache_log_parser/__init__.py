...
class LineDoesntMatchException(ApacheLogParserException):
    ...
    def __repr__(self):
        return u"LineDoesntMatchException(log_line={0!r}, regex={1!r})".format(
            self.log_line, self.regex
        )
...
def extract_inner_value(output_prefix, input_suffix):
    """
    Given an input format like %{Referer}o return a function that will extract
    that 'Referer' from a match
    """
...
def extra_request_from_first_line(matched_strings):
    ...
    match = re.match(
        "^(?P<method>GET|HEAD|POST|OPTIONS|PUT|CONNECT|PATCH|PROPFIND|DELETE)"
        "\s?(?P<url>.{,10000}?)(\s+HTTP/(?P<http_ver>1.[01]))?$",
        first_line
    )
    if match is None:
        ...
        return {
            'request_first_line': first_line,
            'request_method': match.groupdict()['method'],
            'request_url': match.groupdict()['url'],
            'request_http_ver': match.groupdict()['http_ver']
        }
...
def apachetime(s):
    ...
    obj = datetime(year=int(s[7:11]), month=month_map[s[3:6]], day=int(s[0:2]),
                   hour=int(s[12:14]), minute=int(s[15:17]),
                   second=int(s[18:20]), tzinfo=tz)
...
def format_time(matched_strings):
    ...
    return {
        ...
        'time_received_datetimeobj': naive_obj,
        'time_received_isoformat': naive_obj.isoformat(),
        'time_received_tz_datetimeobj': obj,
        'time_received_tz_isoformat': obj.isoformat(),
        'time_received_utc_datetimeobj': utc_obj,
        'time_received_utc_isoformat': utc_obj.isoformat(),
    }
...
FORMAT_STRINGS = [
    [
        ...
        '%',
        lambda match: '',
        lambda matched_strings: matched_strings
    ],
    # Remote IP-address.
    [
        make_regex('%a'),
        IP_ADDR_REGEX,
        lambda match: 'remote_ip',
        lambda matched_strings: matched_strings
    ],
    # Local IP-address.
    [
    ...
    [
        make_regex('%b'),
        '(\d+|-)',
        lambda match: 'response_bytes_clf',
        lambda matched_strings: matched_strings
    ],
    ...
    [
        make_regex('%H'),
        '.*?',
        lambda match: 'protocol',
        lambda matched_strings: matched_strings
    ],
    # Special case of below, for matching just user agent.
    [
        make_regex('%\{User-Agent\}i'),
        '.*?',
        lambda match: "request_header_user_agent",
        parse_user_agent
    ],
    ...
    # modules would have modified it, use mod_setenvif to copy the header into
    # an internal environment variable and log that value with the %\{VARNAME}e
    # described above.
    [
        make_regex('%\{[^\}]+?\}i'),
        '.*?',
        extract_inner_value("request_header_", "i"),
        lambda matched_strings: matched_strings
    ],
    # Number of keepalive requests handled on this connection.
    # Interesting if KeepAlive is being used, so that, for example, a '1' means
    # the first keepalive request after the initial one, '2' the second, etc...
    # otherwise this is always 0 (indicating the initial request).
    # Available in versions 2.2.11 and later.
    [
        make_regex('%k'),
        '.*?',
        lambda match: 'num_keepalives',
        lambda matched_strings: matched_strings
    ],
    ...
    [
        make_regex('%\{[^\}]+?\}n'),
        '.*?',
        extract_inner_value("note_", "n"),
        lambda matched_strings: matched_strings],
    # The contents of Foobar: header line(s) in the reply.
    [
        make_regex('%\{[^\}]+?\}o'),
        '.*?',
        extract_inner_value("response_header_", "o"),
        lambda matched_strings: matched_strings
    ],
    ...
    [
        make_regex('%p'),
        '.*?',
        lambda match: 'server_port',
        lambda matched_strings: matched_strings
    ],
    # The canonical port of the server serving the request or the server actual
    # port or the client's actual port.
    # Valid formats are canonical, local, or remote.
    [
        make_regex('%\{[^\}]+?\}p'),
        '.*?',
        extract_inner_value("server_port_", "p"),
        lambda matched_strings: matched_strings],
    ...
    # Valid formats are pid, tid, and hextid. hextid requires APR 1.2.0 or
    # higher.
    [
        make_regex('%\{[^\}]+?\}P'),
        '.*?',
        extract_inner_value("pid_", "P"),
        lambda matched_strings: matched_strings
    ],
    ...
    [
        make_regex('%r'),
        '.*?',
        lambda match: 'request_first_line',
        extra_request_from_first_line
    ],
    ...
    [
        make_regex('%t'),
        '\[.*?\]',
        lambda match: 'time_received',
        format_time
    ],
    # The time, in the form given by format, which should be in strftime(3)
    # format. (potentially localized)
    [
        make_regex('%\{[^\}]+?\}t'),
        '.*?',
        extract_inner_value("time_", "t"),
        lambda matched_strings: matched_strings
    ],
    # Extension value, e.g. mod_ssl protocol and cipher
    [
        make_regex('%\{[^\}]+?\}x'),
        '.*?',
        extract_inner_value("extension_", "x"),
        lambda matched_strings: matched_strings
    ],
...
class Parser:
        ...
        while True:
            ...
            if regex is not None:
                for format_spec in FORMAT_STRINGS:
                    pattern_regex, log_part_regex, name_func, values_func = \
                        format_spec
                    match = re.match("^{}$".format(pattern_regex), regex)
                    if match:
                        name = name_func(match.group())
                        self.names.append(name)
                        self.functions_to_parse[name] = values_func
                        self.log_line_regex += "(?P<{}>{})".format(
                            name, log_part_regex
                        )
                        break
    ...
    def parse(self, log_line):
        ...
        if match is None:
            raise LineDoesntMatchException(log_line=log_line,
                                           regex=self.log_line_regex.pattern)
        else:
            results = {}
